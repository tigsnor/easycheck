...
현재 JSON 파일 기반 실험/Plate 저장, 범위 선택, 그룹 지정, 희석 계산, Well 결과 기록, TSV 내보내기, 실험 준비 점검, 결과 행렬 일괄 붙여넣기, 실행 취소와 전체 JSON 백업·복원까지 구현했습니다.

다음 코딩 작업은 `실제 파일 공유와 알파 배포 준비`를 추천합니다.

이유:
- 현재 백업과 TSV는 클립보드 복사 방식이므로 Files 앱, AirDrop, 메일로 직접 전달하는 iOS 공유 sheet가 필요합니다.
