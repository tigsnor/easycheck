...
현재 JSON 파일 기반 실험/Plate 저장, 범위 선택, 그룹 지정, 희석 계산, Well 결과 기록, TSV 내보내기, 실험 준비 점검과 Plate reader 결과 행렬 일괄 붙여넣기까지 구현했습니다.

다음 코딩 작업은 `삭제·변경 작업의 안전장치`를 추천합니다.

이유:
- 실험 삭제가 즉시 실행되고 Plate의 그룹/희석 일괄 변경을 되돌릴 수 없어 실제 연구 데이터에 위험합니다.
- 삭제 확인과 실행 취소, Plate 변경 undo, 저장 상태 표시를 먼저 넣어야 안심하고 알파 테스트를 진행할 수 있습니다.
- 이후 전체 JSON 백업/복원과 실제 CSV 파일 공유로 확장합니다.
