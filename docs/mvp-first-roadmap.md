...
현재 JSON 파일 기반 실험/Plate 저장, 화면 맞춤·단계별 확대와 가로 스크롤, 범위 선택, 그룹 지정, 희석 계산, Well 결과 기록, TSV 복사·파일 공유, 실험 준비 점검, 결과 행렬 일괄 붙여넣기, 실행 취소와 전체 JSON 백업 파일 공유·선택 복원까지 구현했습니다.
...
- 배포 이후 실제 사용 피드백을 바탕으로 가로 모드 세부 조정과 기본 결과 분석을 우선순위화합니다.
