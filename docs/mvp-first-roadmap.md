...
다음 코딩 작업은 `로컬 DB와 Experiment repository 구현`을 추천합니다.

이유:
- 현재 홈/상세 UI는 이미 있으나 데이터가 인메모리라 앱 종료 후 사라집니다.
- plate, attachment, template, analysis도 결국 실험 ID와 저장소를 기준으로 연결됩니다.
- 저장소를 먼저 고정하면 이후 기능을 안정적으로 쌓을 수 있습니다.
