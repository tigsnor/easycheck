...
현재 JSON 파일 기반 실험/Plate 저장, 범위 선택, 그룹 지정, 희석 계산, Well 결과 기록, TSV 내보내기, 실험 준비 점검, 결과 행렬 일괄 붙여넣기와 삭제/Plate 변경 실행 취소까지 구현했습니다.

다음 코딩 작업은 `전체 데이터 백업·복원`을 추천합니다.

이유:
- 앱 내부 JSON 파일만으로는 앱 재설치, 기기 변경 또는 파일 손상에 대비할 수 없습니다.
- 실험 목록과 연결 Plate를 하나의 백업 파일로 내보내고 가져오는 기능이 있어야 실제 연구 데이터를 안심하고 축적할 수 있습니다.
- 백업 이후 실제 CSV 파일 공유와 TestFlight 알파 배포로 확장합니다.
