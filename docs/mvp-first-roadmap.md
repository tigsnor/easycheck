...
현재 JSON 파일 기반 실험/Plate 저장, 범위 선택, 그룹 지정, 희석 계산, Well 결과 기록, TSV 내보내기와 실험 준비 점검까지 구현했습니다.

다음 코딩 작업은 `Plate reader 결과 일괄 붙여넣기`를 추천합니다.

이유:
- Well 하나씩 결과를 입력할 수 있지만 96개 결과를 수동 입력하기에는 시간이 오래 걸립니다.
- 행렬 형태 데이터를 붙여넣어 A1-H12에 매핑하면 여자친구분의 알파 테스트에서 실제 결과 기록 시간을 크게 줄일 수 있습니다.
- 일괄 입력 이후 blank correction, replicate 평균, CV 계산으로 자연스럽게 확장할 수 있습니다.
