# PlateNote Flutter 개발 플랜

> 이 문서는 전체 장기 기능 후보를 보관하는 참고 계획입니다. 현재 실행 우선순위와 출시 완료 기준은 [단일 사용자 우선 출시 계획](single-user-release-plan.md)을 따릅니다. 단일 사용자 버전이 실제 실험에서 안정화되기 전에는 계정, cloud sync, 협업, 결제와 고급 분석을 구현하지 않습니다.

## 1. 제품 비전

PlateNote는 생명과학 연구원이 96-well plate 기반 실험을 아이폰 메모앱처럼 빠르게 기록하고, 농도 배치와 serial dilution 계산, 실험군 관리, 사진 기록, 결과 분석까지 한곳에서 관리하는 모바일 우선 실험 노트 앱입니다.
