...
import '../../../shared/presentation/inline_error_panel.dart';
...
import '../application/plate_editor_controller.dart';
...
class _PlateEditorScreenState extends State<PlateEditorScreen> {
  ...
  late final PlateEditorController _editorController;
  Plate? _plate;
  ...
  Object? _saveError;
  Plate? _failedPlate;
  String? _failedSuccessMessage;
  String? _failedChangeSummary;
  bool _failedRecordHistory = true;
  ...
  void initState() {
    ...
    _editorController = PlateEditorController()..addListener(_onEditorChanged);
    _loadPlate();
  }
  ...
  WellPosition? get _selectedPosition => _editorController.selectedPosition;
  Set<WellPosition> get _selectedPositions =>
      _editorController.selectedPositions;
  WellPosition? get _rangeAnchor => _editorController.rangeAnchor;
  double? get _plateCellSize => _editorController.requestedCellSize;
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: _isLoading
            ...
            : plate == null && _loadError != null
            ? _PlateLoadError(onRetry: _loadPlate)
            : plate == null
            ? const Center(child: CircularProgressIndicator())
            : ListView(
  ...
  SaveStatus get _plateSaveStatus {
  ...
  bool get _canZoomPlateOut => _editorController.canZoomOut;

  bool get _canZoomPlateIn => _editorController.canZoomIn;

  void _zoomPlateOut() => _editorController.zoomOut();

  void _zoomPlateIn() => _editorController.zoomIn();
  ...
  void _selectRow(int rowIndex) {
    ...
    _editorController.selectRow(rowIndex, plate.columnCount);
  }
  ...
  void _selectColumn(int columnIndex) {
    ...
    _editorController.selectColumn(columnIndex, plate.rowCount);
  }

  void _clearSelection() => _editorController.clearSelection();
  ...
  Future<void> _chooseAndApplyTemplate() async {
    ...
    final saved = await _savePlate(
      ...
      changeSummary: '템플릿 적용: ${selected.name}',
    );
    if (saved && mounted) {
      _clearSelection();
    }
  ...
  Future<void> _openBulkResultImport() async {
    ...
    await _savePlate(
      ...
      changeSummary: '결과 일괄 입력: ${draft.preview.valueCount}개 well',
    );
  }
  ...
  Future<void> _openDilutionBuilder() async {
    ...
    await _savePlate(updated, changeSummary: '희석 계산 적용: ${group.name}');
  }
  ...
  Future<void> _assignGroupToSelection() async {
    ...
    await _savePlate(
      ...
      changeSummary:
          '그룹 지정: ${group.name} (${_selectedPositions.length}개 well)',
    );
  }
  ...
  Future<void> _editSelectedWell() async {
    ...
    final updatedWells = plate.wells
        ...
        .toList();
    await _savePlate(
      plate.copyWith(wells: updatedWells),
      changeSummary: 'well 수정: ${updated.position.label}',
    );
  }
  ...
  Future<bool> _savePlate(
    ...
    try {
      await widget.repository.savePlate(plateToSave);
    } on Object catch (error) {
      if (!mounted) {
        return false;
      }
      setState(() {
        ...
        _saveError = error;
        _failedPlate = plate;
        _failedSuccessMessage = successMessage;
        _failedChangeSummary = changeSummary;
        _failedRecordHistory = recordHistory;
      });
      return false;
    }
    if (!mounted) {
    ...
    setState(() {
      ...
      _saveError = null;
      _failedPlate = null;
      _failedSuccessMessage = null;
      _failedChangeSummary = null;
    });
  ...
  Future<void> _retryPlateSave() async {
    final plate = _failedPlate;
    if (plate == null || _isSaving) return;
    await _savePlate(
      plate,
      successMessage: _failedSuccessMessage ?? 'plate layout을 저장했습니다.',
      changeSummary: _failedChangeSummary ?? 'Plate 수정',
      recordHistory: _failedRecordHistory,
    );
  }
  ...
  Future<void> _undoLastChange() async {
    ...
    final currentRevisions = _plate?.revisions ?? const <PlateRevision>[];
    final restored = await _savePlate(
      previous.copyWith(
        revisions: [
          ...currentRevisions,
          PlateRevision(
            id: 'plate-revision-${DateTime.now().microsecondsSinceEpoch}',
            changedAt: DateTime.now().toUtc(),
            summary: '마지막 Plate 변경 실행 취소',
          ),
        ],
      ),
...
class _PlateLoadError extends StatelessWidget {
  const _PlateLoadError({required this.onRetry});
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const InlineErrorPanel(
              message: 'Plate를 불러오지 못했습니다. 저장된 데이터는 변경되지 않았습니다.',
            ),
            const SizedBox(height: 12),
            FilledButton.icon(
              key: const ValueKey('retry-plate-load'),
              onPressed: onRetry,
...
class _PlateSaveStatus extends StatelessWidget {
  const _PlateSaveStatus({required this.status, required this.lastSavedAt});
        ...
        child: Column(
          ...
          children: [
            ...
            if (_errorMessage != null) ...[
              const SizedBox(height: 12),
              InlineErrorPanel(message: _errorMessage!),
            ],
            const SizedBox(height: 12),
            Wrap(
              spacing: 8,
...
class _GroupEditorSheetState extends State<_GroupEditorSheet> {
  ...
  void _save() {
    ...
    if (name.isEmpty) {
      setState(() => _nameError = '그룹명을 입력해주세요.');
      return;
    }
